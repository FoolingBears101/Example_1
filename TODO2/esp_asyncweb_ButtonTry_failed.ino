...
/*---- added vars -----*/
  ...
  const String off_str = "on";          // aucune idée pourquoi c'est inversé a la lécture 
                                         //j'ai pourtant vérifier les variables sur le serial
...
void setup(){
  ...
  auto root_handler = server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    for(int i=0;i<paramNb;i++){
      ...
      if(strcmp(p->name().c_str(),green.c_str()) ) 
      {
  ...
  server.on("/light", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    server.on("/redLedSwitch", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    request->send_P(200, "text/plain","switching red led");
  });
    ...
    server.on("/greenLedSwitch", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    request->send_P(200, "text/plain","switching green led");
  });
    ...
    server.on("/overrideOff", HTTP_GET, [](AsyncWebServerRequest *request){
    overRide_state = "off";
    request->send_P(200, "text/plain","switching green led");
  });
...
<body>
  ...
  <p>
    ...
    <button id="redButton" class="button button1">Turn on/off</button>
  </p>
    ...
    <button id="greenButton" class="button button2">Turn on/off</button>
  </p>
    ...
    <button id="bigBlackButton" class="button button3">stop</button>
  </p>
...
document.getElementById("redButton").onclick = function() {
  var xhttp = new XMLHttpRequest();
  xhttp.open("GET", "/redLedSwitch", true);
  xhttp.send();    
...
document.getElementById("greenButton").onclick = function() {
  var xhttp = new XMLHttpRequest();
  xhttp.open("GET", "/greenLedSwitch", true);
  xhttp.send();    
  
document.getElementById("bigBlackButton").onclick = function() {
  var xhttp = new XMLHttpRequest();
  xhttp.open("GET", "/overrideOff", true);
  xhttp.send();    
